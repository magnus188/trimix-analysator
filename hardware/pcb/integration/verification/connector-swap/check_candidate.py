...
def build():
 ...
 target={'J402':(4.45,13.5),'J401':(4.9,27),'R401':(9,18.73),'R403':(1.3,19.48)}
 movements=[]
 for ref,new in target.items():
  f=fps[ref];old=centre(f);pos=f.GetPosition();delta=(new[0]-old[0],new[1]-old[1]);f.SetPosition(vec(p.ToMM(pos.x)+delta[0],p.ToMM(pos.y)+delta[1]));f.BuildCourtyardCaches();movements.append({'ref':ref,'old_physical_centre_mm':old,'new_physical_centre_mm':centre(f),'translation_mm':list(delta),'rotation_unchanged_deg':f.GetOrientationDegrees()})
 modified_edges=0
