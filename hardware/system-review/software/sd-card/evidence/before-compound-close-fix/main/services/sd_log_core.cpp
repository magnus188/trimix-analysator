...
Status Logger::status() const {
    std::lock_guard<std::mutex> guard(lock_);
    auto out=status_; out.dropped+=contention_drops_.load(); return out;
}
...
void Logger::close_current() {
    if(open_generation_) {
        const bool ok=backend_.close(); open_generation_=0;
        if(!ok) { failure("SD close failed; recent data may be incomplete"); return; }
