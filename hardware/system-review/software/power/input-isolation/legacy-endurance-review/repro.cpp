...
struct Bus:system_i2c::Bus {
    ...
    bool bc_pending=false;uint8_t bc_result=0x40;
    Bus() {
    ...
    bool write(uint8_t a,const uint8_t *d,size_t n) override {
        ...
        if(a==0x6a && d[0]==0 && (r[a][0]&0x80) && !(d[1]&0x80))input_woke=clock_ms;
        if(a==0x5f && d[0]==1) {bc_pending=d[1]==0x0a;bc_started=clock_ms;}
    ...
    bool read_register(uint8_t a,uint8_t reg,uint8_t *out,size_t n) override {
        ...
        if(a==0x5f && reg==2 && n==1) {
            *out=bc_pending && uint32_t(clock_ms-bc_started)>50 ? bc_result:0;
...
void loop_observe() {
    if(scenario=="legacy-endurance") {
        if(phase==0 && system_usb_input_sample().permission_confirmed) {button_fn(nullptr);phase=1;}
        else if(phase==1 && system_power_charge_standby()) {check(true,"Legacy DCP reaches charge standby");phase=2;}
        else if(phase==2 && kills) {std::cout<<"Shutdown at virtual ms="<<clock_ms<<"\n";check(false,"Stable DCP must not force shutdown during periodic capability refresh");throw End{};}
        if(clock_ms>=120000) {check(phase==2 && system_power_charge_standby() && !kills,"Stable DCP remains available for two virtual minutes");throw End{};}
        return;
    }
...
int main(int argc,char **argv) {
    if(argc!=2)return 2;scenario=argv[1];if(scenario=="legacy-endurance"){bus.r[0x47][8]=0;bus.bc_result=0x80;}pins[32]=1;worker_thread=std::this_thread::get_id();
    ...
    if(scenario.rfind("hiz-",0)!=0 && scenario!="legacy-endurance")check(off_cycles>=3,"Actual worker exercised persistent off on retained external host power");
