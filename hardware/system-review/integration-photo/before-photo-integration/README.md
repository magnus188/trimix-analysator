...
The approved A3 exterior and the original PCBFit baseline are preserved. Mechanical changes belong to the separate **Trimix_Enclosure_A3_SystemReview** Fusion design. Purchasing, showcase work and production printing remain paused.

**2026-09-08 integration update:** the owner has supplied a rear photograph with device-up confirmed. It locates the Guition header and both USB ports at the top and the onboard SD socket at the left. The [photo registration and complete-harness review](integration-photo/README.md) supersedes the old illustrative connector locations; prior v11 clearance receipts do not prove fit of these newly located interfaces. SD logging and the revised mechanical checkpoint are being implemented and require fresh evidence; the firmware counts below describe the preserved pre-SD verification snapshot. The [bench-equipment record](lab-equipment.md) now includes the confirmed multimeter, scope, DC PSU, DL24/P, DLA and FNB58.
...
The matching **SystemReview v11** is saved and its native archive reopens with the same geometry, parameters and assembly associations. It retains the approved 85 × 180 × 43 mm exterior and exact CNC Kitchen insert models. Two carrier through-openings clear the revised rear components with complete local 2 mm material borders. Checked component, connector, service, gas and fastener allocations pass at 85 and 87 mm widths, with the intended 85 mm design restored. Both oxygen reference configurations pass their bounded checks. See the [final CAD checkpoint](mechanical/final-local-checkpoint/final-handoff.json). Unmeasured plugs, sealing faces and cable bends remain provisional.

Both P4 firmware builds pass, alongside **50 normal and 50 sanitized host tests**, 95 repository checks and 18 static-analysis units. The current schematic/firmware contract passes 70 assertions. CO now requires fresh chamber temperature/humidity within its documented range; invalid conditions clear its displayed value while oxygen and helium stay independent. The [CO verification](software/co-environment/README.md) records the tests and [manufacturer use restrictions](electrical/co-module-use-review.md). This module cannot certify breathing-gas safety.

The STEP is a reference export with a documented limitation (CAD09): body count, bounds and selected surface/section checks pass, but strict volume comparison and one underlying-plane correspondence do not establish full per-body equivalence. The exactly reopened native Fusion archive remains the authoritative mechanical model.

Eight PLA/PETG diagnostic projects for the four affected printed parts pass 50 slice checks. These are fit-study files, with support removal, printed dimensions, retention, sealing and temperature tests still pending. [The acceptance ledger](acceptance.csv) separates completed digital work from the **13 remaining order-blocking gates**; those gates are related and are not 13 demonstrated hardware failures.
