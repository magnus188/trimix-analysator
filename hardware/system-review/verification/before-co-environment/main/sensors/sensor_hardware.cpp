...
void publish(const gas_acquisition::Frame &frame,const oxygen_selection_status_t &selection,float co,bool co_valid) {
    ...
    out.co_valid=co_valid; out.co_ppm=co_valid ? co:NAN;
    ...
    for (int i=0;i<GAS_CAL_CHANNEL_COUNT;++i) {
        ...
        double percent=NAN;
        if (!gas_calibration_convert(channel,r.mean_voltage_v,percent) || !std::isfinite(percent) || percent<0 || percent>100) {
            out.status=SENSOR_STATUS_FAULT; continue;
        }
        if (channel==GAS_CAL_HELIUM) { out.helium_percent=percent; out.helium_calibrated=true; }
        else {
            out.oxygen_percent=percent; out.oxygen_calibrated=true;
            gas_cal_record_t record{};
            if (gas_calibration_get_record(channel,&record)) out.oxygen_calibration_revision=record.revision;
...
void acquisition_task(void *) {
    ...
    while(!stopping.load()) {
        ...
        if(uart_ok && now-boot>=2000) {
            std::lock_guard<std::mutex> guard(state_lock);
            gpio_set_level(co_enable_pin,!stopping.load());
        }
        if(uart_ok) {
            uint8_t bytes[64]; const int count=uart_read_bytes(co_uart,bytes,sizeof(bytes),0);
            for(int i=0;i<count;++i) decoder.push(bytes[i],now);
        }
        float co=NAN;
        const bool co_valid=now-boot>=300000 && decoder.latest(now,co);
        publish(frame,selected,co,co_valid);
