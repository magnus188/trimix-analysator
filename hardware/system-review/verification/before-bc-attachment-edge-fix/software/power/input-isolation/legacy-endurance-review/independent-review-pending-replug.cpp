...
struct Bus:system_i2c::Bus {
    ...
    bool bc_pending=false;uint8_t bc_result=0x40, latched_bc_result=0;
...
void loop_observe() {
    if(scenario=="legacy-pending-replug") {
        const auto source=system_usb_input_sample();
        if(phase==0 && source.bc.status==bc12::Status::Detecting) {
            check(source.cc.change_latched && !pins[50],"Detection starts while initial CC sticky interrupt remains set");
            // End this attachment and connect a different SDP, same orientation
            // and Default advertisement. TUSB still reports its already-set
            // interrupt. PI3 remains HOST-powered and retains its completed
            // read-clear DCP result until an explicit new detection edge.
            bus.r[0x47][9]|=0x10; bus.bc_result=0x40;
            std::cout<<"Boundary swap DCP to SDP at virtual ms="<<clock_ms<<"; retained one-shot result="<<unsigned(bus.latched_bc_result)<<'\n';
            phase=1;return;
        }
        if(phase==1) {
            check(!source.permission_confirmed && !pins[50] &&
                  source.power.input_path==power_monitor::InputPath::Isolated,
                  "A second attachment hidden under initial sticky IRQ must not inherit old DCP permission");
            std::cout<<"state="<<unsigned(source.state)<<" grant="<<source.permission_confirmed
                     <<" current_ma="<<source.requested_limit_ma<<" port="<<unsigned(source.bc.port)
                     <<" detections="<<bus.bc_cycles<<" current_source_result="<<unsigned(bus.bc_result)<<'\n';
            throw End{};
        }
        if(clock_ms>20000){check(false,"Pending-replug boundary reached");throw End{};}
        return;
    }
