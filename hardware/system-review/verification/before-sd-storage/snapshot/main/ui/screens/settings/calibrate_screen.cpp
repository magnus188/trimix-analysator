...
lv_obj_t *matrix(lv_obj_t *parent, const char *const *map, lv_event_cb_t cb) {
    ...
    lv_obj_set_style_bg_color(obj, lv_color_hex(STYLE_COLOR_PRIMARY), LV_PART_ITEMS | LV_STATE_CHECKED);
...
void refresh() {
    ...
    lv_label_set_text(g.mode_label, simulated ? "SIMULATION - practice data" : "HARDWARE - bench calibration");
    if (g.sim_profile) {
...
void timer_cb(lv_timer_t *) { refresh(); }
