...
esp_err_t sensor_read_all(sensor_readings_t *out) {
    ...
    *out = {};
    ...
    for (int i = 0; i < GAS_CAL_CHANNEL_COUNT; ++i) {
        ...
        double calibrated;
        if (gas_calibration_convert(channel, raw.voltage_v, calibrated) && calibrated >= 0 && calibrated <= 100) {
            ...
            if (channel == GAS_CAL_HELIUM) { out->helium_percent = calibrated; out->helium_calibrated = true; }
        }
    }
    return ESP_OK;
#endif
