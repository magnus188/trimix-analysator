...
f.SetPosition(v((13.25,78.75)));f.SetOrientationDegrees(0)
...
def via_hits(a,net='GND',dia=.6):
 ...
 return hits
G=(14.725,78.75);REG=(11.775,78.75)
result={'pose':[13.25,78.75,0],'pack_paths':[],'gnd_to_existing':[],'vias':[],'regn':blocked(REG,(12.79,81.15),'/01  CHARGING + BATTERY/BQ_REGN',.25)}
for points in [[(10.64,79.9091),(10.6,77.4),(13.4,77.4),(16.1271,74.422)],[(10.64,79.9091),(10.65,79.95),(13.25,79.95),(13.65,77.25),(16.1271,74.422)],[(10.64,79.9091),(10.8,80),(13.2,80),(13.2,77.0),(16.1271,74.422)]]:
...
for x in [15.75,16,16.25,16.5,17,17.5,18]:
 for y in [77.25,77.75,78.25,78.75,79.25,79.75]:
  a=(x,y);hits=via_hits(a)
  if not hits:result['vias'].append({'at':a,'line_blocks':blocked(G,a,'GND',.25)})
(D/'trial-geometry.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2))
