...
import pcbnew as p
D=Path(__file__).resolve().parent;sys.path.insert(0,str(D.parents[4]/'hardware/tools'))
...
assert removed=={'03fb83e2-af05-49db-9428-77bb39e6c5ac'} and not m['changed_poses']
...
assert child(old[next(iter(removed))],'net')[1]=='I2C_SDA'
...
assert not overlaps
assert all(q['net']in['I2C_SCL','CHG_INT_N','I2C_SDA']for q in m['added_items'])
...
out={'status':'isolated_additive_proposal_passed_native_DRC_not_board_release','before_sha256':sha(before),'after_sha256':sha(after),'added_segments':sum(q['type']=='segment'for q in m['added_items']),'added_vias':len(vias),'removed_items':len(removed),'modified_existing_copper_without_explicit_replacement':0,'modified_footprints':0,'zones_added':0,'zone_boundaries_rules_and_project_preserved':True,'zones_refilled':True,'new_via_SMD_overlaps':overlaps,'new_via_nearest_land_gaps':gaps,'before_DRC':summary(prior),'after_DRC':summary(post),'introduced_DRC_violations':introduced,'In1_signal_additions':0,'new_trace_width_mm':.15,'new_via_diameter_drill_mm':[.50,.25],'new_via_annulus_mm':.125,'approved_SDA_bridge':{'removed_uuid':'03fb83e2-af05-49db-9428-77bb39e6c5ac','via_centres_mm':[[16.8,79.5193],[17.8,78.5193]],'bridge_layer':'F.Cu','bridge_width_mm':.15},'integration':'Remove only the exact approved SDA node after matching its frozen content, then append the exact new segment/via UUID nodes; do not replace the owner board. Refill zones and rerun native DRC/parity on the merged board. Review actual inner-plane cuts and parent HOST/QON overlays.','limits':['Not an ordering release: other unconnected and dangling findings remain.','No component positions, package dimensions or board outlines changed; exactly one original SDA segment is replaced by the approved local layer bridge.','Native DRC does not qualify noise, power return impedance, temperature, EMI or fabrication.']}
...
out['files']={name:sha(D/name)for name in['delta.json','route-patch.kicad_sexpr','added-items.kicad_sexpr','before-drc.json','after-drc.json','build_upper_bus.py','run_routes.py','route_bounded.py']}
