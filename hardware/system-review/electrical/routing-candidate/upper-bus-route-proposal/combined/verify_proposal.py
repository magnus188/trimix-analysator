...
assert removed=={'03fb83e2-af05-49db-9428-77bb39e6c5ac','3c0670f1-c03a-4a0a-bf6a-33687f86a6cc','e1670b7a-858d-41a6-aa2d-11c5439e603d','8065715f-01f7-4d87-93d1-782d09179296'} and len(m['changed_poses'])==1
...
assert set(child(old[k],'net')[1]for k in removed)=={'I2C_SCL','I2C_SDA','CHG_INT_N'}
...
assert not overlaps
assert all(q['net']in['I2C_SCL','I2C_SDA','CHG_INT_N']for q in m['added_items'])
...
out={'status':'isolated_scoped_proposal_passed_native_DRC_not_board_release','before_sha256':sha(before),'after_sha256':sha(after),'added_segments':sum(q['type']=='segment'for q in m['added_items']),'added_vias':len(vias),'removed_items':len(removed),'modified_existing_copper_without_explicit_replacement':0,'modified_footprints':m['changed_poses'],'zones_added':0,'zone_boundaries_rules_and_project_preserved':True,'zones_refilled':True,'new_via_SMD_overlaps':overlaps,'new_via_nearest_land_gaps':gaps,'before_DRC':summary(prior),'after_DRC':summary(post),'introduced_DRC_violations':introduced,'In1_signal_additions':0,'new_trace_width_mm':.15,'new_via_diameter_drill_mm':[.50,.25],'new_via_annulus_mm':.125,'approved_C707_escape':{'reference':'C707','old_center_mm':[21,69],'new_center_mm':[21.7,68.925],'rotation_deg':90,'package_unchanged':True,'provisional_J301_mated_envelope_X_gap_mm':.72,'height_contract_refresh_required':True},'integration':'Remove only the four explicitly approved original SCL/SDA/CHG segments and replace only C707 with its exact proposed footprint after matching its frozen content, then append the exact new segment/via UUID nodes; do not replace the owner board. Refill zones and rerun native DRC/parity on the merged board. Review actual inner-plane cuts and parent HOST/QON overlays.','limits':['Not an ordering release: other unconnected and dangling findings remain.','No purchased package dimensions or board outlines changed; four reviewed original SCL/SDA/CHG segments are replaced and C707 is moved0.70mmX/−0.075mmY; all other original copper and poses are preserved.','Native DRC does not qualify noise, power return impedance, temperature, EMI or fabrication.']}
assert out['after_DRC']['unconnected']==out['before_DRC']['unconnected']-2
...
out['files']={name:sha(D/name)for name in['delta.json','route-patch.kicad_sexpr','added-items.kicad_sexpr','before-drc.json','after-drc.json','stage-provenance.json']}
