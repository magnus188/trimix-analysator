...
a=Native(sexpdata.loads((D/'before.kicad_pcb').read_text()));b=Native(sexpdata.loads((D/'Trimix_Analyzer.kicad_pcb').read_text()));area=box(7,-89.15,27,-74)
out=['<svg xmlns="http://www.w3.org/2000/svg" width="3240" height="740" viewBox="0 0 81 18.5"><rect width="81" height="18.5" fill="white"/>']
...
for k,(n,L,title)in enumerate([(a,'In2.Cu','Before In2'),(b,'In2.Cu','After In2'),(b,'F.Cu','After F / projected SYS'),(b,'B.Cu','After B / projected SYS')]):
 out.append(f'<g transform="translate({k*20.25-7},-72.8)">')
 for z in n.zones:
 ...
 for q in n.pads_on(L):
  if q['ref']in['U101','C108','C103','Q111','TP1013','R107']and area.covers(Point(q['x'],q['y'])):out.append(f'<text x="{q["x"]}" y="{-q["y"]+.07}" font-size=".19" text-anchor="middle" fill="black">{q["ref"]}.{q["pin"]}</text>')
 out.append(f'<text x="7.1" y="73.5" font-family="Arial" font-size=".36">{title}</text>')
 for yy in[75,80,85]:out.append(f'<text x="7.1" y="{yy}" font-family="Arial" font-size=".18">y{yy}</text>')
 out.append('</g>')
out.append('<text x=".2" y="17.3" font-family="Arial" font-size=".32">Orange: CHG interrupt. Purple: SDA. Cyan: reserved future 0.40mm SYS path (not yet actual copper); outer-layer panels show its projection.</text>')
out.append('<text x=".2" y="18.0" font-family="Arial" font-size=".30">Native filled copper and package pads only. Scope X7..27 / Y74..89.15mm. In1 stays continuous; all six final In2 ground regions retain an In1 anchor.</text></svg>')
