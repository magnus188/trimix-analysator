...
local=box(.6,-89.15,29.2,-65);rows={}
for L in ['In1.Cu','In2.Cu']:
 ...
 rows[L]=dict(before_area_mm2=aa.area,after_area_mm2=bb.area,removed_area_mm2=aa.difference(bb).area,added_area_mm2=bb.difference(aa).area,before_component_areas_mm2=count(aa),after_component_areas_mm2=count(bb),local_review_rectangle=[.6,65,29.2,89.15],local_before_component_areas_mm2=count(al),local_after_component_areas_mm2=count(bl),erosion_probe=[dict(radius_mm=r,before_component_areas_mm2=count(al.buffer(-r)),after_component_areas_mm2=count(bl.buffer(-r)))for r in [.05,.1,.15,.25,.5]])
 oldpins=[]
 for v in a.vias:
