...
for uid in f0:
 if f0[uid]!=f1[uid]:
  ref=next(q[2]for q in children(f0[uid],'property')if q[1]=='Reference');changed.append(ref)
  assert ref=='R111'
  assert child(f0[uid],'at')[1:]==[27.5,77.75,90]
  assert child(f1[uid],'at')[1:]==[27.5,77.55,-90]
  def comparable(fp):
   out=[]
   for q in fp:
    if isinstance(q,list)and str(q[0])=='at':continue
    if isinstance(q,list)and str(q[0])in['property','fp_text']:
     q=[v for v in q if not isinstance(v,list)or str(v[0])!='at']
    if isinstance(q,list)and str(q[0])=='pad':
     q=[v[:3]if isinstance(v,list)and str(v[0])=='at'else v for v in q]
    out.append(sx.dumps(q))
   return sorted(out)
  assert comparable(f0[uid])==comparable(f1[uid]),'Package content changed beyond pose/text orientation'
assert changed==['R111']
...
assert not overlaps and not vias
patch=[sx.Symbol('cc_interrupt_route_patch'),[sx.Symbol('remove')]+[old[k]for k in sorted(removed)],[sx.Symbol('add')]+[new[q['uuid']]for q in m['added_items']],[sx.Symbol('replace_footprint')]+[[f0[k],f1[k]]for k in f0 if f0[k]!=f1[k]]]
...
(OUT/'added-items.kicad_sexpr').write_text(sx.dumps([sx.Symbol('cc_interrupt_additions')]+[new[q['uuid']]for q in m['added_items']])+'\n')
...
assert len(prior['unconnected_items'])==23 and len(post['unconnected_items'])==22
assert not any('R111' in q['description']for row in post['unconnected_items']for q in row['items'])
assert len(added)==5 and len(removed)==4 and all(q['layer']=='F.Cu'and q['width_mm']==.15 for q in m['added_items'])
assert all(child(old[k],'net')[1]=='HOST_3V3' for k in removed)
assert all(q['net']in['HOST_3V3','USB_CC_INT_N'] for q in m['added_items'])
out['resolved_scope']=['USB_CC_INT_N R111.2 to existing U110 interrupt via; HOST branch remains connected']
...
out['files']={name:sha(OUT/name)for name in['delta.json','route-patch.kicad_sexpr','added-items.kicad_sexpr','removed-items.json','before-drc.json','after-drc.json','routing-geometry-audit.json','build_cc_int.py','route_cc_int.py']}
out['reference_label_global_placement_preserved']=True
out['remaining_CC_interrupt_remote_gap']='Unchanged separate U115.3(13.6,89.775) to F branch at24.6127,86.75 remains outside assigned local scope.'
out['scope_extension']='New HOST segment touches its already-routed source at26.85,79.75,0.25mm below nominal assigned y79.5 boundary; no other regions changed.'
