...
import build_cc_int as c
p=c.p; b=c.b
def route(NET,start,end,end_layers=(0,1,2),start_layers=(0,),width=.15,allow_vias=True,bounds=(25.0,75.5,29.0,79.5)):
 ...
 def legal(n):
  if n not in legal_cache:
   q=pt(n);legal_cache[n]=BOUNDS[0]<=q[0]<=BOUNDS[2] and BOUNDS[1]<=q[1]<=BOUNDS[3] and not blocked(q,LAYERS[n[2]],.2001+width/2)
  return legal_cache[n]
 def change(n):
  k=n[:2]
  if k not in via_cache:
   q=pt(n)
   via_cache[k]=q[0]<=29.25 and all(not blocked(q,l,.4501)for l in ALL) and all(not s.Collide(c.vec(q),p.FromMM(.4501))for l in [p.F_Cu,p.B_Cu]for s in shapes(smd,l,q))
  return via_cache[k]
...
route('HOST_3V3',(27.5,76.725),(28.1,77.575),end_layers=(0,),allow_vias=False)
route('HOST_3V3',(26.85,79.75),(29,77.575),end_layers=(0,),allow_vias=False,bounds=(25,75.5,29.5,79.9))
route('USB_CC_INT_N',(27.5,78.375),(26.1,78.9),end_layers=(0,1,2))
c.save()
