...
file=D/(REF+'-0402-B-scout.kicad_pcb');file.write_text(raw);b=p.LoadBoard(str(file));f=next(f for f in b.GetFootprints()if f.GetReference()==REF);f.Flip(f.GetPosition(),False)
...
for q in b.GetFootprints():
 ...
 q.BuildCourtyardCaches();shape=q.GetCourtyard(p.B_Cu)
 if shape.OutlineCount():courts.append((q.GetReference(),shape))
objects=[t for t in b.GetTracks()if t.IsOnLayer(p.B_Cu)]+[q for fp in b.GetFootprints()if fp.GetReference()!=REF for q in fp.Pads()if q.IsOnLayer(p.B_Cu)]
shapes=[(q.GetNetname(),q.GetEffectiveShape(p.B_Cu),getattr(q,'GetUuid',lambda:None)())for q in objects]
rules=[z.Outline()for z in list(b.Zones())+[z for fp in b.GetFootprints()for z in fp.Zones()]if z.GetIsRuleArea()and z.GetLayerSet().Contains(p.B_Cu)and z.GetDoNotAllowFootprints()]
rows=[];rejects=collections.Counter();court_ok=[]
xb,yb,xe,ye=(17.25,53.25,20,55.1) if REF=='R702'else(12.8,80.3,15.4,81.3)
for xi in range(round(xb*20),round(xe*20)+1):
 for yi in range(round(yb*20),round(ye*20)+1):
  x,y=xi/20,yi/20
  for angle in [0,90,180,270]:
   f.SetPosition(vec((x,y)));f.SetOrientationDegrees(angle);f.BuildCourtyardCaches();court=f.GetCourtyard(p.B_Cu)
   ...
   for pad in f.Pads():
    shape=pad.GetEffectiveShape(p.B_Cu);net=pad.GetNetname();pt=xy(pad.GetPosition())
...
(D/(REF+'-B-pose-scout.json')).write_text(json.dumps({'candidates':rows,'court_ok':court_ok,'removed':removed,'rejects':dict(rejects),'unaccepted':True},indent=2)+'\n');print(json.dumps(rows[:35],indent=2));print(rejects)
