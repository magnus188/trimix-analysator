...
import pcbnew as p
D=Path(__file__).resolve().parent;OUT=D/'co-local-fb';shutil.copytree(D/'baseline',OUT)
...
code=code.replace('for l in range(3)if l!=n[2]', 'for l in (0,2)if l!=n[2]')
...
for name,start,end in [('topdivider_to_FB',(17.05,55.375),(18.7875,55.8)),('bottomdivider_to_FB',(17.675,58.65),(17.05,55.375))]:
 try:g['route']('CO_FB',start,end,start_layers=(0,),end_layers=(0,),allow_vias=True,bounds=(14.5,53.35,23,61.5));checks.append({'name':name,'routed':True})
