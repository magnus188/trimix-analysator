...
LOCAL = Path(__file__).resolve().parent / "frozen-local-bundle/review"
spec = importlib.util.spec_from_file_location('electrical_audit', HERE / 'audit_electrical.py')
audit = importlib.util.module_from_spec(spec)
spec.loader.exec_module(audit)
source = ET.parse(LOCAL / 'analyzer-netlist.xml').getroot()
...
(LOCAL / 'audit-fault-tests.json').write_text(json.dumps(receipt, indent=2) + '\n')
