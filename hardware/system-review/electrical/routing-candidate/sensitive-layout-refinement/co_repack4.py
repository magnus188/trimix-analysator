...
import pcbnew as p
D=Path(__file__).resolve().parent;OUT=D/'co-repack4';OUT.mkdir(exist_ok=True)
...
f=fps['C702'];f.SetPosition(vec((21.25,54.2)));f.SetOrientationDegrees(0)
f=fps['R702'];f.SetPosition(vec((18.8,54.25)));f.SetOrientationDegrees(90)
# HE via/body conflict: candidate shift within the local signal; incident endpoints only.
for t in b.GetTracks():
 if t.m_Uuid.AsString()=='1dec4277-ebf3-405f-ad46-ec7af44f839c':t.SetPosition(vec((18.95,53.05)))
 elif t.GetNetname()=='HE_ENABLE':
  if xy(t.GetStart())==(18.6399,53.8876):t.SetStart(vec((18.95,53.05)))
  if xy(t.GetEnd())==(18.6399,53.8876):t.SetEnd(vec((18.95,53.05)))
track('CO_SW',[(20.2125,56.3),(21.25,56.3),(21.25,53.15),new['2']],p.F_Cu,.2)
track('CO_5V28',[(20.3,54.2),(20.2125,54.9296)],p.F_Cu,.25)
track('GND',[(22.2,54.2),(23.35,54.25)],p.F_Cu,.35)
