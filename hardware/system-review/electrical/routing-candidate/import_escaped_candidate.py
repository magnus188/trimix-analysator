...
src=P/'Trimix_Analyzer.kicad_pcb';a=sx.loads((OUT/'escaped-before-routing.kicad_pcb').read_text());latest=sx.loads(src.read_text())
...
b=p.LoadBoard(str(dest));names={n.GetNetname() for n in b.GetNetInfo().NetsByNetcode().values()};ses=(OUT/'escaped.ses').read_text();remaps=[]
def reconcile(m):
 name=m[2]
 if name not in names and '/'+name in names:remaps.append(name);name='/'+name
 return m[1]+name+m[3]
ses=re.sub(r'(\(net\s+")([^"]*)(")',reconcile,ses)
(OUT/'escaped-reconciled.ses').write_text(ses)
assert p.ImportSpecctraSES(b,str(OUT/'escaped-reconciled.ses'))
...
expect=OUT/'escaped-expected-geometry.kicad_pcb';expect.write_text(sx.dumps(latest));old=p.LoadBoard(str(expect))
def geometry(b):
...
(OUT/'escaped-import-receipt.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2))
