"""Validate a scoped VBUS-sense patch against its immutable base."""
...
assert not overlaps
patch=[sx.Symbol('vbus_route_patch'),[sx.Symbol('remove')]+[old[k]for k in sorted(removed)],[sx.Symbol('add')]+[new[q['uuid']]for q in m['added_items']],[sx.Symbol('replace_footprint')]+[[f0[k],f1[k]]for k in f0 if f0[k]!=f1[k]]]
...
(OUT/'added-items.kicad_sexpr').write_text(sx.dumps([sx.Symbol('vbus_route_additions')]+[new[q['uuid']]for q in m['added_items']])+'\n')
...
assert len(prior['unconnected_items'])==36 and len(post['unconnected_items'])==35
assert not any('USB_VBUS_DET' in q['description'] for row in post['unconnected_items'] for q in row['items'])
assert old['222dfe92-badf-43a1-972f-171087a3ac32']==new['222dfe92-badf-43a1-972f-171087a3ac32']
out['resolved_scope']=['USB_VBUS_DET R110.2 to U110.4']
out['preserved_HOST_source_via']={'uuid':'222dfe92-badf-43a1-972f-171087a3ac32','at_mm':[19.9905,70.8601]}
out['HOST_bridge_FCu_mm']=[[19.9905,70.8601],[22.05,70.3],[22.3,70.35],[22.55,70.6],[23.4,73.3],[23.65,75.45],[23.85,75.7],[24.5,75.98]]
...
out['files']={name:sha(OUT/name)for name in['delta.json','route-patch.kicad_sexpr','added-items.kicad_sexpr','removed-items.json','before-drc.json','after-drc.json','routing-geometry-audit.json','build_vbus.py','route_vbus.py']}
