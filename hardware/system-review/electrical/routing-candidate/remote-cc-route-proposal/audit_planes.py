...
for L in ['In1.Cu','In2.Cu']:
 ...
 for region in sorted(polys(bb),key=lambda g:-g.area):
  ...
  pads=[dict(pad=q['id'],touches_In1=g1.distance(q['geo'])<.00001)for q in b.pads if q['net']=='GND'and'*.Cu'in q['layers']and region.distance(q['geo'])<.00001]
