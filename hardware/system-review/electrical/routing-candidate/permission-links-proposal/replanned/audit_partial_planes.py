...
source=sexpdata.loads((D/'before.kicad_pcb').read_text());target=sexpdata.loads((D/'handoff-partial-frozen.kicad_pcb').read_text());a,b=Native(source),Native(target)
...
out=dict(source_sha256=hashlib.sha256((D/'before.kicad_pcb').read_bytes()).hexdigest(),candidate_sha256=hashlib.sha256((D/'handoff-partial-frozen.kicad_pcb').read_bytes()).hexdigest(),layers=rows,notes=['Filled-zone geometry only; counts exclude copper traces, pads and barrels that can join islands.','A rectangle intersection or erosion can split a connected outside region; probe results are geometric diagnostics, not measured current capacity or a guaranteed minimum neck width.','All In1 and In2 outlines/settings are preserved; new via clearances affect both planes.','Candidate adapted to frozen HOST source; acceptance still requires parent review.'])
(D/'handoff-partial-plane-audit.json').write_text(json.dumps(out,indent=2)+'\n')
