...
p=c.p; b=c.b
def route(NET,start,end,end_layers=(0,1,2),start_layers=(0,),width=.15,allow_vias=True,bounds=(.6,86.8,11.5,96.2)):
 ...
 reservations=[]
 for at in [(13.6,91),(13,91.65),(16.7,87.95)]:
  v=p.PCB_VIA(b);v.SetPosition(c.vec(at));v.SetWidth(p.FromMM(.6 if at==(13,91.65) else .5));v.SetDrill(p.FromMM(.3 if at==(13,91.65) else .25));v.SetViaType(p.VIATYPE_THROUGH);v.SetLayerPair(p.F_Cu,p.B_Cu);reservations.append(v)
  for L in ALL:add_shape(index,L,v.GetEffectiveShape(L))
 for L,points in [(p.F_Cu,[(13.6,91),(15.2,89.7),(16.7,87.95)]),(p.B_Cu,[(13.675,90.975),(13.6,91)]),(p.B_Cu,[(16.7,87.95),(17.5,87.2217)])]:
  for a,z in zip(points,points[1:]):
   t=p.PCB_TRACK(b);t.SetStart(c.vec(a));t.SetEnd(c.vec(z));t.SetWidth(p.FromMM(.15));t.SetLayer(L);reservations.append(t);add_shape(index,L,t.GetEffectiveShape(L))
 ...
 def candidates(q,l):
  ...
  return out
 ends={n:d for l in end_layers for n,d in candidates(end,l)}
 ...
 current.append(end);paths.append((layer,current))
