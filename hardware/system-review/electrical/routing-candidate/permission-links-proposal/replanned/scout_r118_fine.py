...
for side in ['F','B']:
 L=side+'.Cu';obs=[q for q in n.pads_on(L)if q['ref']!='R118']+[q for q in n.tracks if q['layer']==L]+n.vias
 ...
 othercy=[courtyard(f,side)for ref,f in fps.items()if ref!='R118'];othercy=[q for q in othercy if not q.is_empty]
 ...
 for ang in [0,90,180,270]:
  for xi in range(300,401):
   for yi in range(1810,1900):
    at=(xi/20,yi/20)
    ...
    results.append(dict(side=side,at=at,angle=ang,score=math.dist(at,(16.1827,91.7017))))
results.sort(key=lambda q:q['score']);(D/'r118-fine-scout.json').write_text(json.dumps(results,indent=2));print('total',len(results));print(json.dumps(results[:30],indent=2))
