...
raw=sexpdata.loads((D/'q110-turn-base.kicad_pcb').read_text());n=Native(raw)
...
fp=next(q for q in n.fps if q['ref']=='Q110');pp=[q for q in n.pads if q['ref']=='Q110'];oldcy=courtyard(fps['Q110'],'F');results=[]
for side in ['F']:
 L=side+'.Cu';obs=[q for q in n.pads_on(L)if q['ref']not in ['Q110']]+[q for q in n.tracks if q['layer']==L]+n.vias
 ...
 othercy=[courtyard(f,side)for ref,f in fps.items()if ref not in ['Q110']];othercy=[q for q in othercy if not q.is_empty]
 ...
 for ang in [90]:
  for xi in range(300,411):
   for yi in range(1760,1821):
    at=(xi/20,yi/20)
    ...
    results.append(dict(side=side,at=at,angle=ang,score=math.dist(at,(17.0,89.5))))
results.sort(key=lambda q:q['score']);(D/'q110-180-scout.json').write_text(json.dumps(results,indent=2));print('total',len(results));print(json.dumps(results[:30],indent=2))
