...
D=Path(__file__).resolve().parent;path=D/'q110-core-frozen.kicad_pcb';b=p.LoadBoard(str(path));g=NativeGraph(b,stackup(path.read_text()))
rows=[]
for A,Z in [('Q110.1','U112.5'),('Q110.2','Q111.3')]:
...
(D/'q110-core-witnesses.json').write_text(json.dumps(dict(source_sha256=hashlib.sha256(path.read_bytes()).hexdigest(),pairs=rows,method='Native direct adjacency; no assumed IC or same-pin internal joins; no zones in witness paths.'),indent=2))
