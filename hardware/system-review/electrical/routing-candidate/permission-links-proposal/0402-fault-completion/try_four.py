...
for ref,at,ang,side in [('R116',(10.6,91.95),0,'F'),('R119',(5.5,88.5),180,'F'),('R118',(17.4,94.7),90,'B')]:
...
def ground(ref):
 q=pad(ref,'2');a=c.xy(q.GetPosition());L=p.B_Cu if q.IsOnLayer(p.B_Cu) else p.F_Cu;ALL=[p.F_Cu,p.In1_Cu,p.In2_Cu,p.B_Cu];obs=list(b.GetTracks())+[q for f in b.GetFootprints()for q in f.Pads()]
...
c.save();(D/'four-results.json').write_text(json.dumps(results,indent=2)+'\n')
