...
for side in ['F']:
 ...
 for ang in [15,30,45,60,75,105,120,135,150,165,195,210,225,240,255,285,300,315,330,345]:
  for xi in range(160,251):
   for yi in range(1720,1851):
    at=(xi/20,yi/20)
...
results.sort(key=lambda q:q['score']);(D/'r116-angle-scout.json').write_text(json.dumps(results,indent=2));print('total',len(results));print(json.dumps(results[:30],indent=2))
