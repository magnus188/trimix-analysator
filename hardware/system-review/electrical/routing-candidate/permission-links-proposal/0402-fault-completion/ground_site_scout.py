...
for x in range(350,421):
 for y in range(1720,1790):
...
(D/'ground-sites.json').write_text(json.dumps(out,indent=2));print('count',len(out));print('near1',sorted(out,key=lambda q:q['distance1'])[:15]);print('near2',sorted(out,key=lambda q:q['distance2'])[:15])
