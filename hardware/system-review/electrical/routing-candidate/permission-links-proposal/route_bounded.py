...
def route(NET,start,end,end_layers=(0,1,2),start_layers=(0,),width=.15,allow_vias=True,bounds=(.6,86.8,11.5,96.2)):
 ...
 if NET=='USB_ILIM_SERIES':
  virtualfp=p.FOOTPRINT(b);virtual=p.PAD(virtualfp);virtual.SetPosition(c.vec((18,89.2)));virtual.SetSize(c.vec((4.6,3.4)));virtual.SetShape(p.PAD_SHAPE_RECT);add_shape(index,p.F_Cu,virtual.GetEffectiveShape(p.F_Cu))
