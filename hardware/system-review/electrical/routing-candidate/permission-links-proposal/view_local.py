...
n=Native(sexpdata.loads((D/'before.kicad_pcb').read_text()))
...
for idx,L in enumerate(['F.Cu','B.Cu']):
 ...
 out.append('</g>')
out.append('</svg>');(D/'local-pocket.svg').write_text(''.join(out));cairosvg.svg2png(url=str(D/'local-pocket.svg'),write_to=str(D/'local-pocket.png'))
