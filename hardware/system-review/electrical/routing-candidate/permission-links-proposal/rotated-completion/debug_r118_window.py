...
fp=next(q for q in n.fps if q['ref']=='R118');pp=[q for q in n.pads if q['ref']=='R118'];oldcy=courtyard(fps['R118'],'B');results=[]
for side in ['B']:
 L=side+'.Cu';obs=[q for q in n.pads_on(L)if q['ref']not in ['R118']]+[q for q in n.tracks if q['layer']==L]+n.vias
 ...
 othercy=[courtyard(f,side)for ref,f in fps.items()if ref not in ['R118']];othercy=[q for q in othercy if not q.is_empty]
 ...
 for ang in [90,270]:
  for xi in [490]:
   for yi in [1660,1664,1666,1670]:
    at=(xi/20,yi/20)
    def trans(g):return affinity.translate(affinity.rotate(g,ang-fp['rotation'],origin=(fp['x'],fp['y'])),at[0]-fp['x'],-at[1]-fp['y'])
    g=trans(oldcy)
    hits=ct.query(g.buffer(-.00001),predicate='intersects')
    if len(hits):print('COURTYARD',at,ang,[othercy[int(i)].bounds for i in hits]);continue
    pads=[dict(q,geo=trans(q['geo']))for q in pp]
    hits=[(p.get('pin'),obs[int(i)].get('ref'),obs[int(i)].get('net'),list(obs[int(i)]['geo'].bounds),p['geo'].distance(obs[int(i)]['geo'])) for p in pads for i in tr.query(p['geo'].buffer(.20001)) if obs[int(i)]['net']!=p['net'] and p['geo'].distance(obs[int(i)]['geo'])<.20001]
    if hits:print('COPPER',at,ang,hits);continue
    if any(any(p['geo'].distance(n.holes[int(i)]['geo'])<.25001 for i in holes.query(p['geo'].buffer(.25001)))for p in pads):continue
    results.append(dict(side=side,at=at,angle=ang,score=math.dist(at,(22.0427,87.1732))))
