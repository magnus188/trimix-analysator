...
def route(NET,start,end,end_layers=(0,1,2),start_layers=(0,),width=.15,allow_vias=True,target_points=None,bounds=(.6,86.8,11.5,96.2)):
 ...
 reservations=[]
 for at in [(13,91.65),(7.1,90.35),(10.5,90.5)]:
  v=p.PCB_VIA(b);v.SetPosition(c.vec(at));v.SetWidth(p.FromMM(.6));v.SetDrill(p.FromMM(.3));v.SetViaType(p.VIATYPE_THROUGH);v.SetLayerPair(p.F_Cu,p.B_Cu);reservations.append(v)
  for L in ALL:add_shape(index,L,v.GetEffectiveShape(L))
 for L,points,w in [(p.F_Cu,[(6.15,90.5),(7.1,90.35)],.4),(p.F_Cu,[(8.85,90.5),(8.85,91.45),(8.85,92.2)],.4),(p.F_Cu,[(8.85,89.55),(9.2,89.55),(10.5,90.5)],.25)]:
  for a,z in zip(points,points[1:]):
   t=p.PCB_TRACK(b);t.SetStart(c.vec(a));t.SetEnd(c.vec(z));t.SetWidth(p.FromMM(w));t.SetLayer(L);reservations.append(t);add_shape(index,L,t.GetEffectiveShape(L))
 ...
 def change(n):
  ...
  if k not in via_cache:
   q=pt(n)
   via_cache[k]=not any(s.Collide(c.vec(q),p.FromMM(.7501))for s in shapes(edge,p.F_Cu,q)) and all(not s.Collide(c.vec(q),p.FromMM(.2501))for l in ALL for s in shapes(vrule,l,q)) and all(not blocked(q,l,.4501)for l in ALL) and all(not s.Collide(c.vec(q),p.FromMM(.4501))for l in [p.F_Cu,p.B_Cu]for s in shapes(smd,l,q))
  return via_cache[k]
