...
def route(NET,start,end,end_layers=(0,1,2),start_layers=(0,),width=.15,allow_vias=True,target_points=None,bounds=(.6,86.8,11.5,96.2)):
 ...
 def change(n):
  ...
  if k not in via_cache:
   q=pt(n)
   via_cache[k]=not any(s.Collide(c.vec(q),p.FromMM(.7251))for s in shapes(edge,p.F_Cu,q)) and all(not s.Collide(c.vec(q),p.FromMM(.2251))for l in ALL for s in shapes(vrule,l,q)) and all(not blocked(q,l,.4251)for l in ALL) and all(not s.Collide(c.vec(q),p.FromMM(.4251))for l in [p.F_Cu,p.B_Cu]for s in shapes(smd,l,q))
  return via_cache[k]
