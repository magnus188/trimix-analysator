...
raw=sexpdata.loads((D/'r116-scout-base.kicad_pcb').read_text());n=Native(raw)
...
for side in ['F']:
 L=side+'.Cu';obs=[q for q in n.pads_on(L)if q['ref']not in ['R116']]+[q for q in n.tracks if q['layer']==L]+n.vias
 ...
 othercy=[courtyard(f,side)for ref,f in fps.items()if ref not in ['R116']];othercy=[q for q in othercy if not q.is_empty]
 ...
 for ang in [0,90,180,270]:
  for xi in range(260,591):
   for yi in range(1720,1880):
    at=(xi/20,yi/20)
    ...
    results.append(dict(side=side,at=at,angle=ang,score=math.dist(at,(18,90))))
results.sort(key=lambda q:q['score']);(D/'r116-front-right-scout.json').write_text(json.dumps(results,indent=2));print('total',len(results));print(json.dumps(results[:30],indent=2))
