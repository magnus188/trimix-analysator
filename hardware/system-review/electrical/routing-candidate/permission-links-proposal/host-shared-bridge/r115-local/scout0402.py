...
raw=sexpdata.loads((D/'r115-0402-scout.kicad_pcb').read_text());n=Native(raw)
...
def courtyard(fp,side):
 ...
 for q in fp:
  ...
  if t=='fp_line':lines.append(LineString([(sub(q,k)[0],-sub(q,k)[1])for k in ['start','end']]))
   ...
   x,y=sub(q,'center');xx,yy=sub(q,'end');chunks.append(Point(x,-y).buffer(math.hypot(xx-x,yy-y)))
  elif t=='fp_rect':
...
fp=next(q for q in n.fps if q['ref']=='R115');pp=[q for q in n.pads if q['ref']=='R115'];oldcy=courtyard(fps['R115'],'F');results=[]
for side in ['F']:
 L=side+'.Cu';obs=[q for q in n.pads_on(L)if q['ref']not in ['R115']]+[q for q in n.tracks if q['layer']==L]+n.vias
 # Include reserved outside-SMD root UVLO and raw-source via corridors.
 for x,y,r in []:obs.append(dict(net='RESERVED',geo=Point(x,-y).buffer(r)))
 if side=='F':
  for a,b in []:obs.append(dict(net='RESERVED',geo=LineString([a,b]).buffer(.075)))
 othercy=[courtyard(f,side)for ref,f in fps.items()if ref not in ['R115']];othercy=[q for q in othercy if not q.is_empty]
 ...
 for ang in [0,90,180,270]:
  for xi in range(380,591):
   for yi in range(1540,1770):
    at=(xi/20,yi/20)
    ...
    results.append(dict(side=side,at=at,angle=ang,score=math.dist(at,(23,83.5))))
results.sort(key=lambda q:q['score']);(D/'r115-0402-upper-scout.json').write_text(json.dumps(results,indent=2));print('total',len(results));print(json.dumps(results[:30],indent=2))
