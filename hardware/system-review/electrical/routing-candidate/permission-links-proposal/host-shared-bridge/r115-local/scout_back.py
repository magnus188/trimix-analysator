...
raw=sexpdata.loads((D/'pruned-scout.kicad_pcb').read_text());n=Native(raw)
...
for side in ['B']:
 ...
 for ang in [0,90,180,270]:
  for xi in range(494,547):
   for yi in range(1696,1758):
    at=(xi/20,yi/20)
    ...
    results.append(dict(side=side,at=at,angle=ang,score=math.dist(at,(26,86.75))))
results.sort(key=lambda q:q['score']);(D/'r115-back-scout.json').write_text(json.dumps(results,indent=2));print('total',len(results));print(json.dumps(results[:30],indent=2))
