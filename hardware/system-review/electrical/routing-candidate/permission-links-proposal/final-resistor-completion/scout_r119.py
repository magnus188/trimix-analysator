...
raw=sexpdata.loads((D/'scout-base-v2.kicad_pcb').read_text());n=Native(raw)
...
fp=next(q for q in n.fps if q['ref']=='R119');pp=[q for q in n.pads if q['ref']=='R119'];oldcy=courtyard(fps['R119'],'F');results=[]
for side in ['F']:
 ...
 for x,y,r in [(13,91.65,.30),(16.4,90.3,.25)]:obs.append(dict(net='RESERVED',geo=Point(x,-y).buffer(r)))
 if side=='F':
 ...
 for ang in [0,90,180,270]:
  for xi in range(120,241):
   for yi in range(3520,3611):
    at=(xi/40,yi/40)
...
results.sort(key=lambda q:q['score']);(D/'r119-local-scout.json').write_text(json.dumps(results,indent=2));print('total',len(results));print(json.dumps(results[:30],indent=2))
