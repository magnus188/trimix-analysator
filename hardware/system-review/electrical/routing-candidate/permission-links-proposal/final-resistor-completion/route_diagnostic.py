...
p=c.p; b=c.b
def route(NET,start,end,end_layers=(0,1,2),start_layers=(0,),width=.15,allow_vias=True,target_points=None,bounds=(.6,86.8,11.5,96.2)):
 LAYERS=[p.F_Cu,p.In2_Cu,p.B_Cu]; ALL=[p.F_Cu,p.In1_Cu,p.In2_Cu,p.B_Cu]
 STEP=.025; BOUNDS=bounds
 index={l:{}for l in ALL}; smd={l:{}for l in ALL}; smdown={l:{}for l in ALL}; edge={l:{}for l in ALL}; rule={l:{}for l in ALL}; vrule={l:{}for l in ALL}
 ...
 for it in list(b.GetTracks())+[q for f in b.GetFootprints()for q in f.Pads()]:
  for layer in ALL:
   ...
   if layer==p.In2_Cu and it.GetNetname()=='HOST_3V3':continue
   shape=it.GetEffectiveShape(layer)
   if it.GetNetname()!=NET:add_shape(index,layer,shape)
   if isinstance(it,p.PAD) and it.GetAttribute()==p.PAD_ATTRIB_SMD:add_shape(smdown if it.GetNetname()==NET else smd,layer,shape)
 # Explicit reservations for concurrently proposed UVLO, raw-source and OVP sense copper.
 reservations=[]
 for at,diam,drill in [((13,91.65),.6,.3),((16.4,90.3),.5,.25)]:
  v=p.PCB_VIA(b);v.SetPosition(c.vec(at));v.SetWidth(p.FromMM(diam));v.SetDrill(p.FromMM(drill));v.SetViaType(p.VIATYPE_THROUGH);v.SetLayerPair(p.F_Cu,p.B_Cu);reservations.append(v)
  for L in ALL:add_shape(index,L,v.GetEffectiveShape(L))

 # Owner reserves the C116 back-side adjustment; no new B tracks or through vias here.
 ...
 add_shape(rule,p.B_Cu,reserved);add_shape(vrule,p.B_Cu,reserved)
 ...
 def candidates(q,l):
  ...
  return out
 target_points=target_points or [(end,l) for l in end_layers]
 ends={}
 for endpoint,l in target_points:
  for n,d in candidates(endpoint,l):
   if n not in ends or d<ends[n][0]:ends[n]=(d,endpoint)
 ...
 existing_vias={tuple(round(x,6)for x in c.xy(v.GetPosition()))for v in b.GetTracks()if isinstance(v,p.PCB_VIA)and v.GetNetname()==NET}
 def change(n):
  if pt(n)in existing_vias:return True
  k=n[:2]
  if k not in via_cache:
   q=pt(n)
   via_cache[k]=not any(s.Collide(c.vec(q),p.FromMM(.7501))for s in shapes(edge,p.F_Cu,q)) and all(not s.Collide(c.vec(q),p.FromMM(.2501))for l in ALL for s in shapes(vrule,l,q)) and all(not blocked(q,l,.4501)for l in ALL) and all(not s.Collide(c.vec(q),p.FromMM(.4501))for l in [p.F_Cu,p.B_Cu]for s in shapes(smd,l,q)) and all(not s.Collide(c.vec(q),p.FromMM(.3001))for l in [p.F_Cu,p.B_Cu]for s in shapes(smdown,l,q))
  return via_cache[k]
 dist={};prev={};queue=[];serial=itertools.count()
 for n,d in [q for l in start_layers for q in candidates(start,l)]:
  if legal(n):dist[n]=d;prev[n]=None;heapq.heappush(queue,(d+math.dist(pt(n),end),next(serial),n))
 goal=None
 while queue:
  ...
  for x,y,l,cost in ns:
   ...
   dist[q]=nd;prev[q]=n;heapq.heappush(queue,(nd+math.dist(pt(q),end),next(serial),q))
 if goal is None:
