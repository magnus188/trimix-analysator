...
def route(NET,start,end,end_layers=(0,1,2),start_layers=(0,),width=.15,allow_vias=True,bounds=(7,74,26.5,86.5),targets=None,starts=None):
 ...
 def change(n):
  ...
  if k not in via_cache:
   q=pt(n)
   via_cache[k]=not any(s.Collide(c.vec(q),p.FromMM(.8001))for s in shapes(edge,p.F_Cu,q)) and all(not s.Collide(c.vec(q),p.FromMM(.3001))for l in ALL for s in shapes(vrule,l,q)) and all(not blocked(q,l,.5001)for l in ALL) and all(not s.Collide(c.vec(q),p.FromMM(.3501))for l in [p.F_Cu,p.B_Cu]for s in shapes(smd,l,q))
  return via_cache[k]
