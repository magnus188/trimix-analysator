...
def main():
 ...
 result={'source_sha256':c.EXPECTED,'status':'HYPOTHETICAL_CC_TRACKS_IGNORED' if ignored else 'ADDITIVE_ALL_FOREIGN_COPPER_PRESERVED','ignored_foreign_track_uuids':sorted(ignored),'native_lower_island':lower,'native_upper_island':upper,'track_width_mm':.4,'via_diameter_mm':args.via_diameter,'via_drill_mm':VR,'all_SMT_PTH_pad_lands_masked_with_005_margin':True,'grid_mm':STEP,'bounds_mm':BOUNDS,'found':found is not None,'visited':len(seen),'elapsed_seconds':time.time()-started,'reachable_grid_nodes':list(seen),'layers':{c.b.GetLayerName(L):{'count':len([n for n in seen if n[2]==i]),'legal_via_sites':[pt(n)for n in seen if n[2]==i and via_cache.get(n[:2],False)]}for i,L in enumerate(LAYERS)}}
