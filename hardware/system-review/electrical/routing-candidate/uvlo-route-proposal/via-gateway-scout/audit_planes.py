...
source=sexpdata.loads((D/'before-refilled.kicad_pcb').read_text());target=sexpdata.loads((D/'Trimix_Analyzer.kicad_pcb').read_text());a,b=Native(source),Native(target)
local=box(6.5,-98.5,24,-84.5);rows={}
for L in ['In1.Cu','In2.Cu']:
 ...
 rows[L]=dict(before_area_mm2=aa.area,after_area_mm2=bb.area,removed_area_mm2=aa.difference(bb).area,added_area_mm2=bb.difference(aa).area,before_component_areas_mm2=count(aa),after_component_areas_mm2=count(bb),local_review_rectangle=[6.5,84.5,24,98.5],local_before_component_areas_mm2=count(al),local_after_component_areas_mm2=count(bl),erosion_probe=[dict(radius_mm=r,before_component_areas_mm2=count(al.buffer(-r)),after_component_areas_mm2=count(bl.buffer(-r)))for r in [.05,.1,.15,.25,.5]])
 oldpins=[]
 for v in a.vias:
...
out=dict(source_sha256=hashlib.sha256((D/'before-refilled.kicad_pcb').read_bytes()).hexdigest(),candidate_sha256=hashlib.sha256((D/'Trimix_Analyzer.kicad_pcb').read_bytes()).hexdigest(),layers=rows,notes=['Filled-zone geometry only; counts exclude copper traces, pads and barrels that can join islands.','A rectangle intersection or erosion can split a connected outside region; probe results are geometric diagnostics, not measured current capacity or a guaranteed minimum neck width.','All In1 and In2 outlines/settings are preserved; new via clearances affect both planes.','UVLO gateway uses an ordinary OVLO replacement and longer HOST F/In2/B bypass; acceptance requires parent topology review.'])
