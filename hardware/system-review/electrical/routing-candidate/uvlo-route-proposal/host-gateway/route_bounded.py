...
def route(NET,start,end,end_layers=(0,1,2),start_layers=(0,),width=.15,allow_vias=True,bounds=(7,74,26.5,86.5),targets=None,starts=None):
 ...
 # Reserve an ordinary UVLO via and the owner's .60 raw-power via.
 for center,radius in [((13.6,91.0),.25),((13.0,91.65),.30)]:
  keep=p.SHAPE_POLY_SET();keep.NewOutline()
  for k in range(96):
   angle=2*math.pi*k/96;rr=radius/math.cos(math.pi/96)
   keep.Append(p.FromMM(center[0]+rr*math.cos(angle)),p.FromMM(center[1]+rr*math.sin(angle)))
