...
p=c.p; b=c.b
def route(NET,start,end,end_layers=(0,1,2),start_layers=(0,),width=.15,allow_vias=True,bounds=(7,74,26.5,86.5),targets=None,starts=None):
 ...
 # Other helper's proposed SERIES escape, not added to the saved board.
 seg=p.PCB_TRACK(b);seg.SetStart(c.vec((17.2625,90.15)));seg.SetEnd(c.vec((16,88.4)));seg.SetWidth(p.FromMM(.15));seg.SetLayer(p.F_Cu)
 add_shape(index,p.F_Cu,seg.GetEffectiveShape(p.F_Cu))
 keep=p.SHAPE_POLY_SET();keep.NewOutline()
 for x,y in [(15.75,88.15),(16.25,88.15),(16.25,88.65),(15.75,88.65)]:keep.Append(p.FromMM(x),p.FromMM(y))
 for L in ALL:add_shape(index,L,keep)
 ...
 def change(n):
  ...
  return via_cache[k]
 dist={};prev={};queue=[];serial=itertools.count();source_origins={}
 source_points=starts or [(l,start)for l in start_layers]
 for layer,source in source_points:
  for n,d in candidates(source,layer):
   if legal(n) and d<dist.get(n,float('inf')):
    dist[n]=d;prev[n]=None;source_origins[n]=source;heapq.heappush(queue,(d+heuristic(pt(n)),next(serial),n))
 goal=None
 while queue:
 ...
 nodes.reverse();paths=[];current=[source_origins[nodes[0]],pt(nodes[0])];layer=nodes[0][2]
