...
before=D/'before.kicad_pcb';after=D/'Trimix_Analyzer.kicad_pcb'
assert sha(before)=='e62c867a411b3405b8ad550557ffd93f6c5e90cd4419dc4b36545040d038bc6a'
...
assert not fpc, 'No footprint changes authorized'
for k in fpc:
 q1,q2=fpo[k],fpn[k]
...
assert sha(D/'Trimix_Analyzer.kicad_pro')==sha(D/'frozen-project.json')
assert sha(D/'Trimix_Analyzer.kicad_dru')==sha(D/'frozen-rules.kicad_dru')
...
allowed={'USB_OVP_UVLO'}
assert len(removed)==0 and all(child(old[k],'net')[1]=='CHG_INT_N' for k in removed)
assert len(added)==6 and all(child(new[k],'net')[1]=='USB_OVP_UVLO' for k in added)
...
assert not p1['schematic_parity']
def finding(q):return(q['type'],q['description'],tuple(sorted(i['uuid']for i in q['items'])))
assert {finding(q)for q in p1['violations']}<={finding(q)for q in p0['violations']},'Introduced finding'
assert all(q['type']in ['track_dangling','via_dangling','silk_over_copper','silk_overlap']for q in p1['violations'])
