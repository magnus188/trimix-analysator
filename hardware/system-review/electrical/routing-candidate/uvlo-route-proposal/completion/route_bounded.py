...
def route(NET,start,end,end_layers=(0,1,2),start_layers=(0,),width=.15,allow_vias=True,bounds=(7,74,26.5,86.5),targets=None,starts=None):
 ...
 # Owner's reserved raw-input through via, not saved by this proposal.
 keep=p.SHAPE_POLY_SET();keep.NewOutline()
 for k in range(96):
  angle=2*math.pi*k/96;rr=.30/math.cos(math.pi/96)
  keep.Append(p.FromMM(13+rr*math.cos(angle)),p.FromMM(91.65+rr*math.sin(angle)))
