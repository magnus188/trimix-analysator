...
files=['before.kicad_pcb','Trimix_Analyzer.kicad_pcb'];area=box(7,-97,28.5,-81)
colors={'HOST_3V3':'#de7d17','GND':'#aacbae','USB_5V':'#e19f5a','USB_OVP_5V':'#2769bc','USB_CHG_5V':'#c1407b','USB_PERMISSION_Q':'#8657a5'}
rows=[];natives=[]
for f in files:
...
def draw(g,c):
 ...
 return s
s=['<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 90 36" width="2520" height="1008"><rect width="90" height="36" fill="white"/>']
for k,n in enumerate(natives):
 for j,L in enumerate(['F.Cu','B.Cu','In1.Cu','In2.Cu']):
  s.append(f'<g transform="translate({j*22.5-7},{k*18-80})">')
  for z in n.zones:
  ...
  for q in n.pads_on(L):
   if q['geo'].intersects(area)and q['ref'] in ['U101','U114','U115','C114','C115','C116','R115','U113','C112']:
    s.append(f'<text x="{q["x"]}" y="{-q["y"]+.06}" font-size=".22" text-anchor="middle">{q["ref"]}.{q["pin"]}</text>')
  s.append(f'<text x="7.1" y="80.8" font-size=".45">{["BEFORE","PROPOSAL"][k]} {L}</text></g>')
