...
def route(NET,start,end,end_layers=(0,1,2),start_layers=(0,),width=.15,allow_vias=True,bounds=(.6,86.8,11.5,96.2)):
 ...
 def legal(n):
  if pt(n)[1]>94.525 and pt(n)[0]<6.075:return False
 ...
 def change(n):
  ...
  if k not in via_cache:
   q=pt(n)
   via_cache[k]=not(q[1]>94.35 and q[0]<6.25) and q[0]<=29.25 and all(not blocked(q,l,.4501)for l in ALL) and all(not s.Collide(c.vec(q),p.FromMM(.4501))for l in [p.F_Cu,p.B_Cu]for s in shapes(smd,l,q))
  return via_cache[k]
...
route('HOST_3V3',(10.175,93.7),(2.3507,90.7157),end_layers=(0,))
c.save()
