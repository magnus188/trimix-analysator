...
def build_mask(layer, via=False):
 ...
 for f in b.GetFootprints():
  for q in f.Pads():
   ...
   bb=q.GetBoundingBox();box=tuple(p.ToMM(t)for t in [bb.GetX(),bb.GetY(),bb.GetRight(),bb.GetBottom()]);rect(box,a.via_diameter/2+.025 if via and q.GetNetname()==a.net else margin)
...
if not found:
 ...
 for L in range(3):
  ...
  im=Image.fromarray(arr[1240:1641,100:601]);im=im.resize((1002,802));dr=ImageDraw.Draw(im);dr.text((8,8),b.GetLayerName(layers[L]),fill='red');gx=(goal[1]-100)*2;gy=(goal[0]-1240)*2;dr.ellipse((gx-7,gy-7,gx+7,gy+7),outline='red',width=2);panels.append(im)
