...
step=.05;x0=8;y0=47;nx=261;ny=301;blocked=np.zeros((ny,nx),dtype=bool);margin=.201+a.width/2
  ...
  for q in f.Pads():
   if q.GetNetname()==a.net:
    if via and q.GetAttribute()==p.PAD_ATTRIB_SMD:
     bb=q.GetBoundingBox();box=tuple(p.ToMM(t)for t in [bb.GetX(),bb.GetY(),bb.GetRight(),bb.GetBottom()]);rect(box,a.via_diameter/2+.025)
    continue
