...
before = OUT/'before.kicad_pcb'
after = OUT/'Trimix_Analyzer.kicad_pcb'
manifest = json.loads((OUT/'added-items.json').read_text())
assert sha(before) == manifest['before_sha256']
assert sha(after) == manifest['after_sha256']
...
ids = {q['uuid'] for q in manifest['added_items']}
...
assert len(prior['unconnected_items']) == 47 and len(post['unconnected_items']) == 42
assert all(not any('BME_VIN' in t['description'] or 'R601' in t['description'] or ('GND' in t['description'] and t['pos']['y'] < 35) for t in q['items']) for q in post['unconnected_items'])
...
patch = [sx.Symbol('upper_route_patch')] + [new[q['uuid']] for q in manifest['added_items']]
...
out = {
    ...
    'resolved_scope':['BME_VIN R601/R602 to existing R603/R604/J601/C601 network','HOST_3V3 R601 to existing R802 network','C402 GND cluster to In1','U401 GND cluster to In1'],
    'new_signal_track_width_mm':.15,'ground_track_width_mm':.2,
    'new_signal_via_diameter_drill_mm':[.5,.25],'new_ground_via_diameter_drill_mm':[.6,.3],
    'minimum_nominal_new_via_annulus_mm':.125,
    'no_new_In1_signal_tracks':True,
    'integration':'Append only the32 segment/via nodes in added-items.kicad_sexpr to the owner board, after checking original UUIDs and scoped copper. Refill ground zones and rerun native DRC/parity against the owner’s combined changes. Do not replace the owner board wholesale.',
    'limits':['Five scoped residual connections completed;42 other missing connections and31 prior dangling warnings remain in this frozen isolated board.','Ground-plane continuity, fabrication qualification, circuit performance and physical fit remain broader project gates.'],
    'files':{name:sha(OUT/name)for name in ['added-items.json','added-items.kicad_sexpr','before-drc.json','after-drc.json','routing-geometry-audit.json']}
}
