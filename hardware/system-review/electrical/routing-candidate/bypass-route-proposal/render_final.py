...
f=D/'Trimix_Analyzer.kicad_pcb';n=Native(sexpdata.loads(f.read_text()))
...
s.append('<text x="0.2" y="10.6" font-size=".25">Raw USB orange / protected USB blue / ground green / DVDT violet. Exact native geometry; isolated final routing review.</text></svg>')
ss=''.join(s);(D/'power-final.svg').write_text(ss);cairosvg.svg2png(bytestring=ss.encode(),write_to=str(D/'power-final.png'))
(D/'power-final-render.json').write_text(json.dumps({'board':str(f),'sha256':hashlib.sha256(f.read_bytes()).hexdigest(),'scope':'F/B/In1 exact copper and selected pads; final isolated bypass proposal; whole board incomplete'},indent=2)+'\n')
