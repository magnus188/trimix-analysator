...
ap=argparse.ArgumentParser();ap.add_argument('--board',type=Path,default=D/'native/Trimix_Analyzer.kicad_pcb');ap.add_argument('--net',required=True);ap.add_argument('--start',required=True);ap.add_argument('--end',required=True);ap.add_argument('--output',type=Path,required=True);ap.add_argument('--width',type=float,default=.15);ap.add_argument('--layer',choices=['In2.Cu','B.Cu','F.Cu'],default='In2.Cu');ap.add_argument('--start-layer',choices=['F.Cu','B.Cu','In2.Cu','all'],default='all');ap.add_argument('--end-layer',choices=['F.Cu','B.Cu','In2.Cu','all'],default='all');ap.add_argument('--outer-only',action='store_true');ap.add_argument('--via-diameter',type=float,default=.5);ap.add_argument('--via-drill',type=float,default=.25);ap.add_argument('--bounds',help='optional xmin,ymin,xmax,ymax routing region');ap.add_argument('--allow-local-ground-slots',action='store_true');ap.add_argument('--block',action='append',default=[]);a=ap.parse_args();layer={'In2.Cu':p.In2_Cu,'B.Cu':p.B_Cu,'F.Cu':p.F_Cu}[a.layer];b=p.LoadBoard(str(a.board));start=tuple(map(float,a.start.split(',')));end=tuple(map(float,a.end.split(',')))
step=.025;x0=0;y0=17;nx=1201;ny=3281;blocked=np.zeros((ny,nx),dtype=bool);margin=.201+a.width/2
...
def build_mask(layer, via=False):
 ...
 for f in b.GetFootprints():
  for q in f.Pads():
   if q.GetNetname()==a.net and not via:continue
   if not via and not q.IsOnLayer(layer)and not(q.GetDrillSize().x or q.GetDrillSize().y):continue
   bb=q.GetBoundingBox();box=tuple(p.ToMM(t)for t in [bb.GetX(),bb.GetY(),bb.GetRight(),bb.GetBottom()]);pad_margin=a.via_diameter/2+.025 if via and q.GetNetname()==a.net else margin
   cx,cy=xy(q.GetPosition());pw,ph=xy(q.GetSize());shape=q.GetShape();angle=math.radians(q.GetOrientationDegrees())
   if shape==p.PAD_SHAPE_CIRCLE:circle(cx,cy,pw/2+pad_margin)
   elif shape==p.PAD_SHAPE_OVAL:
    # True stadium outline; its bounding rectangle incorrectly blocks the
    # already DRC-clean diagonal escape beside circular/oval harness lands.
    if pw>=ph:dx,dy=(pw-ph)/2,0
    else:dx,dy=0,(ph-pw)/2
    rx=dx*math.cos(angle)+dy*math.sin(angle);ry=-dx*math.sin(angle)+dy*math.cos(angle)
    line((cx-rx,cy-ry),(cx+rx,cy+ry),min(pw,ph)/2+pad_margin)
   elif shape==p.PAD_SHAPE_ROUNDRECT and abs(q.GetOrientationDegrees()%90)<.000001:
    radius=min(pw,ph)*q.GetRoundRectRadiusRatio()
    rect((box[0]+radius,box[1]+radius,box[2]-radius,box[3]-radius),radius+pad_margin)
   else:rect(box,pad_margin)
 ...
 for z in b.Zones():
  ...
  elif a.allow_local_ground_slots and layer==p.In2_Cu and z.GetNetname()=='GND':continue
...
vm=blocked.copy()
if a.bounds:
 bx0,by0,bx1,by1=map(float,a.bounds.split(','));X=x0+np.arange(nx)[None,:]*step;Y=y0+np.arange(ny)[:,None]*step;outside=(X<bx0)|(X>bx1)|(Y<by0)|(Y>by1);masks[:,outside]=True;vm[outside]=True
for spec in a.block:
 label,*r=spec.split(',');bx0,by0,bx1,by1=map(float,r);X=x0+np.arange(nx)[None,:]*step;Y=y0+np.arange(ny)[:,None]*step;inside=(X>=bx0)&(X<=bx1)&(Y>=by0)&(Y<=by1);masks[[b.GetLayerName(L) for L in layers].index(label),inside]=True;vm[inside]=True
def cell(pt):return(int(round((pt[1]-y0)/step)),int(round((pt[0]-x0)/step)))
bridges={}
for t in b.GetTracks():
 if isinstance(t,p.PCB_VIA) and t.GetNetname()==a.net:bridges[cell(xy(t.GetPosition()))]=xy(t.GetPosition())
for f in b.GetFootprints():
 for q in f.Pads():
  if q.GetNetname()==a.net and (q.GetDrillSize().x or q.GetDrillSize().y):bridges[cell(xy(q.GetPosition()))]=xy(q.GetPosition())
...
while queue:
 ...
 if (y,x)in bridges:moves +=[(LL,y,x,0)for LL in range(3)if LL!=L]
 elif x%2==0 and y%2==0 and not vm[y,x]:moves +=[(LL,y,x,60)for LL in range(3)if LL!=L]
...
if not found:
 ...
 panels=[]
 for L in range(3):
  arr=np.full((ny,nx,3),245,dtype=np.uint8);arr[done[L]]=[150,225,190];arr[masks[L]]=[50,55,65]
  yy0,xx0=cell((bx0,by0)) if a.bounds else (1240,100)
  yy1,xx1=cell((bx1,by1)) if a.bounds else (1640,600)
  crop=arr[yy0:yy1+1,xx0:xx1+1];im=Image.fromarray(crop).resize((1002,802));dr=ImageDraw.Draw(im);dr.text((8,8),b.GetLayerName(layers[L]),fill='red');gx=(goal[1]-xx0)*1002/crop.shape[1];gy=(goal[0]-yy0)*802/crop.shape[0];dr.ellipse((gx-7,gy-7,gx+7,gy+7),outline='red',width=2);panels.append(im)
 canvas=Image.new('RGB',(3006,802),'white')
 for j,im in enumerate(panels):canvas.paste(im,(j*1002,0))
...
# Existing via centres are not necessarily on the search grid. Snap both
# ends of every layer transition to the same actual bridge centre, including
# transitions at the requested start/end; changing only pts[0]/pts[-1]
# previously produced unequal coordinates across a via.
exact_by_cell=dict(bridges)
exact_by_cell[cell(start)]=start
exact_by_cell[cell(end)]=end
pts=[(L,*exact_by_cell.get(cell((x,y)),(x,y))) for L,x,y in pts]
assert pts[0][1:]==start and pts[-1][1:]==end
newvias=[];segments=[]
def v(q):return p.VECTOR2I(p.FromMM(q[0]),p.FromMM(q[1]))
for s,e in zip(pts,pts[1:]):
 if s[0]!=e[0]:
  assert s[1:]==e[1:]
  if cell(s[1:])in bridges:continue
