...
def capture():
    import adsk
    app,doc,d=review.owned();before=snapshot(d)
    old_camera=app.activeViewport.camera
    visibility=[(o,o.isLightBulbOn)for o in d.rootComponent.allOccurrences]
    ...
    files=[]
    try:
        for o in d.rootComponent.occurrences:o.isLightBulbOn=o.component.partNumber=='TMX-A3-C01'
        for c,key,value in extras:setattr(c,key,False)
        for o in d.rootComponent.allOccurrences:
            if o.component.attributes.itemByName(review.GROUP,'photo_geometry'):
                o.isLightBulbOn=o.component.attributes.itemByName(review.GROUP,'geometry_role').value!='clearance_envelope'
        for name,delta in [('guition-photo-rear.png',(0,0,40)),('guition-photo-oblique.png',(-14,8,40))]:
            cam=app.activeViewport.camera;cam.cameraType=core.CameraTypes.OrthographicCameraType
            target=(4.25,6.36,.8);cam.target=core.Point3D.create(*target)
            ...
            path=review.OUT/name;options=core.SaveImageFileOptions.create(str(path))
            options.width=1200;options.height=1800;options.isAntiAliased=True
            options.isBackgroundTransparent=False
            require(app.activeViewport.saveAsImageFileWithOptions(options),'Image capture failed')
            files.append({'file':str(path),'sha256':review.sha(path)})
    finally:
        for entity,value in visibility:
            if entity.isValid:entity.isLightBulbOn=value
        for entity,key,value in extras:
            if entity.isValid:setattr(entity,key,value)
        app.activeViewport.camera=old_camera;adsk.doEvents();app.activeViewport.refresh()
    review.write('photo-inspection-views.json',{'status':'actual_native_views',
        'files':files,'geometry_poses_preserved':before==snapshot(d),'visibility_restored':all(o.isLightBulbOn==v for o,v in visibility if o.isValid),
        'caption':'Actual native reference geometry. Rear portrait: header at top, both USB mouths +Z, SD socket left. PCB outline and component Z except13.4mm bare tips remain illustrative.'})
