"""Read-only dimensional investigation of the placement-v2 STEP outliers.
...
DEST = BASE/'placement-checkpoint-v2'
SOURCE = DEST/'Trimix_Enclosure_A3_SystemReview_PlacementV2.step'
SOURCE_SHA = '2242c8409e00b7c867fd75c3984958c0dec39556e0ef6b1a21af04feea817e63'
...
def run():
    ...
    if set(original)!=set(NAMES):raise RuntimeError('Expected three unique source bodies')
    ...
    try:
        ...
        result = {'generated_at_utc':datetime.now(timezone.utc).isoformat(),'source_step':str(SOURCE),
            'source_sha256':SOURCE_SHA,'parts':[],
        ...
        if result:
            ...
            (DEST/'verification/step-precision-diagnostic.json').write_text(json.dumps(result,indent=2)+'\n')
            print(json.dumps({'report':str(DEST/'verification/step-precision-diagnostic.json'),
...
def inspect_unmatched_plane():
    """Inspect the native-only analytic plane before interpreting its absence."""
    configure()
    import audit_a3 as audit
    app,doc,design=owned()
    manager=fusion.TemporaryBRepManager.get()
    body=next(manager.copy(body)for _,c,_,body in audit._instances(design)if c.name==NAMES[2])
    target=23.75776695296637
    rows=[]
    for index,face in enumerate(body.faces):
        g=face.geometry
        if g.objectType!=core.Plane.classType():continue
        normal=_normal(g.normal.asArray())
        distance=sum(a*x*10 for a,x in zip(normal,g.origin.asArray()))
        if max(abs(a-b)for a,b in zip(normal,(0,0,1)))>1e-6 or abs(distance-target)>1e-6:continue
        rows.append({'face_index':index,'area_mm2':face.area*100,'bounds_mm':_bounds(face),
                     'point_on_face_mm':[x*10 for x in face.pointOnFace.asArray()],
                     'edge_lengths_mm':[edge.length*10 for edge in face.edges],
                     'vertices_mm':[[x*10 for x in vertex.geometry.asArray()]for vertex in face.vertices]})
    result={'document':doc.name,'inspected_plane_Z_mm':target,'faces':rows,
            'persistent_mutation':False}
    (DEST/'verification/step-unmatched-plane.json').write_text(json.dumps(result,indent=2)+'\n')
