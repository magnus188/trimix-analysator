# Trimix analyser — software-calibration electronics revision

Open `pcb/analyzer/Trimix_Analyzer.kicad_pro`. The root schematic is a
...
ZE07-CO, and a momentary-button power controller. P1 in `pcb/power` and
...
See [SOFTWARE_CALIBRATION.md](SOFTWARE_CALIBRATION.md) for the current ADC/calibration implementation, verification and physical test checklist.

## Oxygen inputs

AO2 and the standalone JJ sensor have separate differential signal paths
into U401, an ADS122C04IPWR at address 0x40. They are multiplexed, not sampled
...
input limits. Gain 8 uses the PGA; both ADCs start at 20 samples/s conversion rate. Sequential channels, diagnostics and settling reduce the delivered per-channel rate. Nominal converter resolution is not measured gas accuracy.
...
[TI ADS122C04 datasheet](https://www.ti.com/lit/ds/symlink/ads122c04.pdf)
...
The other bridge arm is RN501, a matched 2 kΩ / 2 kΩ fixed divider (Vishay ACASN2001U2001P1AT). U502, an ADS122C04IPWR at 0x41, measures HE_REF minus HE_SENSE. Start at gain 1, PGA bypass, internal 2.048 V reference. Software calibration replaces the trimmer. Keep the separate excitation/2 and O₂-bias diagnostic inputs. Verify 3.0 ±0.1 V at the actual sensor
...
Real ADS122C04 acquisition, per-channel calibration/NVS persistence and a receive-only ZE07-CO decoder are implemented and build-tested. The simulator remains separate. The actual PCB has not been tested with this firmware. Physical BME280, battery-gauge/charger and shutdown integration remain pending; physical environmental values are unavailable rather than simulated.
...
I2C addresses: MAX17048 **0x36**, O₂ ADS122C04 **0x40**, He ADS122C04 **0x41**,
