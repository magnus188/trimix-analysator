...
BOARD=HW/'kicad/analyzer/Trimix_Analyzer.kicad_pcb'
MODELS=Path('/Applications/KiCad/KiCad.app/Contents/SharedSupport/3dmodels')

def run():
    b=p.LoadBoard(str(BOARD));parts=[];thermal=[];cache={}
    frame=json.loads((HW/'pcb-a3/verification/main-step-frame.json').read_text())
    ...
    out=HW/'pcb-a3/verification/main-through-hole-relief.json';out.write_text(json.dumps(result,indent=2)+'\n')
