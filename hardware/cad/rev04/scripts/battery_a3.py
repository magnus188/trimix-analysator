"""Revision04 / A3 occupied battery and disconnect reference geometry.
...
from build_a3 import (GROUP, box, checkpoint, circle_xz, extrude_axis,
...
def _mark(component, role, note, appearance):
    ...
    component.attributes.add(GROUP, "physical_group", "disconnect" if role.startswith("disconnect") else "battery")
    component.attributes.add(GROUP, "purchased", "true")
...
def build_battery():
    ...
    expected = {
        ...
        "HolderX": 5.4, "HolderY": 27.0, "HolderZ": 16.5,
    }
    ...
    body = box(plug, "Red battery disconnect reference body", "23 mm", "110 mm", "28 mm",
               "12 mm", "10 mm", "4 mm")
    ...
    box(plug, "Illustrative disconnect key groove", "28.4 mm", "109.9 mm", "27.9 mm",
        "1.2 mm", "10.2 mm", "0.7 mm", "cut", body)
    _mark(plug, "disconnect_body", "Reference envelope X23..35 Y110..120 Z28..32; exact latch, contact and wire geometry unmeasured", red)

    mate = new(NAMES["mate"],
               "RCY/BEC-style red disconnect mating reference; provisional 12 x 10 x 2.2 mm, not selected manufacturer CAD")
    box(mate, "Red battery disconnect mating reference", "23 mm", "110 mm", "32 mm",
        "12 mm", "10 mm", "2.2 mm")
    _mark(mate, "disconnect_mate", "Reference envelope X23..35 Y110..120 Z32..34.2; rearward unplugging clearance requires final service audit", red)
    ...
    print(json.dumps({
        ...
        "holder_bounds_mm": {"min": [5.4, 27, 16.5], "max": [47.4, 107.4, 36.85]},
        "cell_centres_xz_mm": [[16.2, 26.75], [36.6, 26.75]],
        "cell_y_extent_mm": [32, 101],
        "pocket_floor_mm": 17.6,
