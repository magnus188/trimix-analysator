...
#define TAG "TRIMIX_MAIN"

const i2c_port_t i2c_master_port = I2C_NUM_0;

// Touch screen functions (adapted from template)
esp_err_t gt911_init_i2c(void) {
    ...
    ESP_LOGI(TAG, "Initializing Touch I2C");
...
uint16_t gt911_map(uint16_t n, uint16_t in_min, uint16_t in_max, uint16_t out_min, uint16_t out_max) { 
...
void lcd_init(void *arg) {
    ...
    esp_lcd_rgb_panel_config_t panel_config = {
        ...
        .data_gpio_nums = {
            LCD_PIN_DATA0, LCD_PIN_DATA1, LCD_PIN_DATA2,
    ...
    esp_err_t sensor_ret = sensor_interface_init();
    if (sensor_ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to initialize sensors: %s", esp_err_to_name(sensor_ret));
        // Continue anyway - sensors might not be connected during development
    } else {
    ...
    // Initialize Trimix screens
    ESP_LOGI(TAG, "Initializing Trimix screens");
    screens_init();

    ESP_LOGI(TAG, "Trimix Analyzer initialized successfully");
    ESP_LOGI(TAG, "Starting main UI loop");
...
void app_main(void) {
    ESP_LOGI(TAG, "=== Trimix Analyzer ESP32 Starting ===");
    
    // Start LVGL and UI on core #1
    xTaskCreatePinnedToCore(lcd_init, "lcd_init", 16384, NULL, 1, NULL, 1);
